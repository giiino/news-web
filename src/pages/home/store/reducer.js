    ...
    switch (action.type){
        case actionTypes.AJAX_TO_HOMESECTION:
            return state.merge({
                ...
                combineList:fromJS([...action.covid19_data,...action.taiwan_data,...action.china_data,...action.usa_data,...action.anime_data]),
                loading:false
            })
        case actionTypes.LIKE_ARTICLE:
            return state.set('beLikeArticle', state.get('beLikeArticle').push(action.item))
        case actionTypes.DISLIKE_ARTICLE:
            return state.set('beLikeArticle', state.get('beLikeArticle').filter(item => item.title !== action.id))
        default:
            return state
    }
}
